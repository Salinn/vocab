...
SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |primary|
    ...
    #
    #primary.item :key_1, 'name', root_path #, options

    # Add an item which has a sub navigation (same params, but with block)
    #primary.item :key_2, 'name', root_path #, options do |sub_nav|
      # Add an item to the sub navigation (same params again)
      #sub_nav.item :key_2_1, 'name', root_path, options
   # end

    # You can also specify a condition-proc that needs to be fullfilled to display an item.
    # Conditions are part of the options. They are evaluated in the context of the views,
    # thus you can use all the methods and vars you have available in the views.
    #primary.item :key_3, 'Users', root_path #, class: 'special', if: -> { current_user.admin? }
    #primary.item :key_4, 'Courses', root_path #, unless: -> { logged_in? }
