class Grade < ActiveRecord::Base
  ...
  def self.add_or_update(user, answer)
    ...
    lesson_module_total_time = Answer.where(user_id: user.id, question_id: lesson_module.questions).pluck(:time_to_complete).sum
    ...
    if lesson_module_grade.nil?
      Grade.create!(user_id: user.id, lesson_module_id: lesson_module.id, grade: lesson_module_total_grade, time: lesson_module_total_time)
    else
      lesson_module_grade.update_attributes(grade: lesson_module_total_grade, time: lesson_module_total_time)
    end
    ...
    if lesson_grade.nil?
      Grade.create!(user_id: user.id, lesson_id: lesson.id, grade: lesson_total_grade, time: lesson_total_time)
    else
      lesson_grade.update_attributes(grade: lesson_total_grade, time: lesson_total_time)
    end
    ...
    course_total_time = 0
    lesson_grades.each do |lg|
      course_total_time += lg.time
      course_total_grade += lg.grade
    end
    ...
    if course_grade.nil?
      Grade.create!(user_id: user.id, course_id: course.id, grade: course_total_grade, time: course_total_time)
    else
      course_grade.update_attributes(grade: course_total_grade, time: course_total_time)
    end
  end
end
